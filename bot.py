from parsers import parser_today, parser_weather, parser_yesterday, parser_pr_thisweek, list_pr, item_names, parser_item_thisweek, parser_item_nextweek
from config import *
from keyboard import greet_kb, item_kb
...
async def item_timetable(message: types.message):
    chat_id = message.chat.id
    await bot.send_message(chat_id=chat_id, text='пожалуйста, выберите предмет, чье наличие вы хотите проверить в расписании.', reply_markup=item_kb)
#блок, который обрабатывает получаемый от пользователя название предмета
@dp.message_handler(lambda message: message.text in item_names)
async def handler_item(message: types.message):
    chat_id = message.chat.id
    text_thisweek = parser_item_thisweek(url, header, message.text)
    text_nextweek = parser_item_nextweek(url, header, message.text)
    await bot.send_message(chat_id=chat_id, text=f'эта неделя:\n {text_thisweek}', parse_mode='html', reply_markup=ReplyKeyboardRemove())
    await bot.send_message(chat_id=chat_id, text=f'следующая неделя:\n {text_nextweek}', parse_mode='html')
